...
class DefinitionController(context: Context,
    attrs: AttributeSet? = null, @AttrRes defStyleAttr: Int = 0) : StandardVideoController(context,
    attrs, defStyleAttr) {
  ...
  override fun initView() {
    ...
    mPopupWindow = PopupWindow(ViewGroup.LayoutParams.WRAP_CONTENT,
        ViewGroup.LayoutParams.WRAP_CONTENT)
    mPopLayout = LayoutInflater.from(context).inflate(R.layout.layout_rate_pop, this,
  ...
  private fun showRateMenu() {
    ...
    mPopupWindow!!.showAsDropDown(multiRate,
        -((mPopLayout!!.measuredWidth - multiRate!!.measuredWidth) / 2),
        -(mPopLayout!!.measuredHeight + multiRate!!.measuredHeight + WindowUtil.dp2px(context,
            10f)))
  }
  ...
  override fun setProgress(): Int {
    if (multiRate != null && TextUtils.isEmpty(multiRate!!.text)) {
      L.d("multiRate")
      val multiRateData = (mediaPlayer as DefinitionMediaPlayerControl).getDefinitionData()
          ?: return super.setProgress()
      mRateStr = ArrayList()
      mRateItems = ArrayList()
      var index = 0
      val iterator = ArrayList(multiRateData.entries).listIterator(multiRateData.size)
      L.d(""+ iterator)
      while (iterator.hasPrevious()) {//反向遍历
        val entry = iterator.previous()
        mRateStr!!.add(entry.key)
        val rateItem = LayoutInflater.from(context).inflate(R.layout.layout_rate_item,
            null) as TextView
        rateItem.text = entry.key
        rateItem.tag = index
      ...
      if((mRateItems as ArrayList<TextView>).size > 0){
